...
const router=useRouter()
  ...
  return (
    ...
    <FaHeart size={20} />
      {token?("Add to Wishlist"):(<Link className='w-full' href="/auth/Login">Add to Wishlist</Link>)}
  </button>):(<ButtonLoaders ty={'change'} />)} </div>
          <div className='w-full'> {!isLoaded?(  <button className="flex w-full bg-green-600 text-white font-semibold py-3 rounded-lg hover:bg-green-700 transition duration-200" onClick={AddToCart}>
      {token? Incart : (<Link  className='w-full'  href="/auth/Login">Add To Cart</Link>)}
    </button>):(<ButtonLoaders ty={'change'} />)}</div>
